...
const timeTable = [
    ...
    { num: 1, start: "8:30", end: "9:10" }, 
    { num: 2, start: "9:20", end: "10:00" },
...
function renderLoop() {
    ...
    blobs.forEach((blob) => {
        ...
        radialGrad.addColorStop(0, blob.color + '66'); 
        radialGrad.addColorStop(0.4, blob.color + '11'); 
...
window.addEventListener('deviceorientation', e => {
    ...
    gyroY = Math.min(Math.max(e.gamma / 1.5, -12), 12); 
    gyroX = Math.min(Math.max((e.beta - 50) / 1.5, -12), 12);
    document.querySelectorAll('.timer-card, .day-schedule-box, .week-matrix-box').forEach(card => {
        card.style.transform = `rotateX(${gyroX}deg) rotateY(${gyroY}deg)`;
    });
});
function handleStart(clientX, clientY, isTouch, e) {
    ...
    if (isTouch && e.touches && e.touches.length === 2 && currentIdx === 1 && e.target.closest('.week-matrix-box')) {
        isZuming = true; isPanning = false; isDragging = false; 
...
function handleMove(clientX, clientY, isTouch, e) {
    if (isZuming && isTouch && e.touches && e.touches.length === 2 && currentIdx === 1) {
        ...
        matrixScale = Math.min(Math.max(matrixScale * factor, 1.0), 2.5); 
        document.getElementById('matrix-grid').style.transform = `translate3d(${panX}px, ${panY}px, 0) scale(${matrixScale})`; 
...
function handleEnd() {
    ...
    if (dragDirection === 'horizontal') { 
        ...
        if (movedBy < -80 && currentIdx < 1) currentIdx++; 
        if (movedBy > 80 && currentIdx > 0) currentIdx--; 
        ...
        if (lastY > 55) { 
            ...
            pullIndicator.style.transform = 'translate3d(-50%, 60px, 0)'; 
            setTimeout(() => location.reload(true), 600); 
        } else { 
            pullIndicator.style.transform = 'translate3d(-50%, 0, 0)'; 
...
function switchScreen(index) {
    currentIdx = index; 
    currentTranslate = currentIdx * -window.innerWidth; 
    prevTranslate = currentTranslate;
    
    const sDay = document.getElementById('slide-day'), sWeek = document.getElementById('slide-week');
    sDay.style.transition = sWeek.style.transition = 'all 0.5s cubic-bezier(0.16, 1, 0.3, 1)'; 
    swiper.style.transform = `translateX(${currentTranslate}px)`;
    
    if (index === 0) { sDay.style.opacity = '1'; sWeek.style.opacity = '0'; }
    else { sDay.style.opacity = '0'; sWeek.style.opacity = '1'; }
...
function buildMatrix() {
    ...
    activeDays.forEach(d => {
        let cell = document.createElement('div'); cell.className = 'matrix-cell header'; 
        cell.innerText = currentData[d] ? currentData[d].short : "Вс"; 
    ...
    for (let l = startLessonIdx; l <= maxLessonsCount; l++) {
        ...
        activeDays.forEach(d => {
            ...
            const dayObj = currentData[d];
            const name = (dayObj && dayObj.lessons) ? dayObj.lessons[l] : null; 
...
function updateLogic() {
    ...
    if (isMusicMode === 1 && currentUser === 0) { isWeekend = false; }

    if (!isWeekend && currentData[day] && currentData[day].lessons) {
        const lessonsKeys = Object.keys(currentData[day].lessons).map(Number);
        if (lessonsKeys.length > 0) {
            const lastLessonNum = Math.max(...lessonsKeys);
            const lastLessonObj = activeTimeTable.find(t=>t.num===lastLessonNum);
            if (lastLessonObj && currentMinutes >= parseTime(lastLessonObj.end)) { 
                targetDay = day + 1; 
                if (isMusicMode === 1 && currentUser === 0) { if (targetDay > 6) targetDay = 0; }
                else { if (targetDay > 5 || targetDay === 0) targetDay = 1; }
            }
        }
    ...
    let prefixTitle = isMusicMode === 1 ? "🎵 " : "";
    
    const dayTitleEl = document.getElementById('day-title');
    if (dayTitleEl && activeDayInfo) {
        dayTitleEl.innerText = isDisplayingToday ? `${prefixTitle}Сегодня (${activeDayInfo.name})` : `${prefixTitle}След. уч. день (${activeDayInfo.name})`;
    }
    ...
    if (isDisplayingToday && currentData[day] && currentData[day].lessons) {
        ...
        if (lessonsKeys.length > 0) {
            const firstLessonNum = lessonsKeys[0];
            const firstLessonTime = activeTimeTable.find(t=>t.num===firstLessonNum);
            const firstLessonStart = firstLessonTime ? parseTime(firstLessonTime.start) : 0;
            ...
            if (currentMinutes < firstLessonStart) {
                ...
                for (let lNum of lessonsKeys) {
                    let tBox = activeTimeTable.find(t=>t.num===lNum); 
                    if (tBox) {
                        let startM = parseTime(tBox.start), endM = parseTime(tBox.end);
                        if (currentMinutes >= startM && currentMinutes < endM) {
                            ...
                            lessonProgressPercent = (currentMinutes * 60 + currentSecs - startM * 60) / (endM * 60 - startM * 60); 
                            break;
                        }
                    }
                }
                if (activeLessonId === null) {
                    for (let i = 0; i < lessonsKeys.length - 1; i++) {
                        let currEnd = parseTime(activeTimeTable.find(t=>t.num===lessonsKeys[i]).end);
                        let nextStart = parseTime(activeTimeTable.find(t=>t.num===lessonsKeys[i+1]).start);
                        if (currentMinutes >= currEnd && currentMinutes < nextStart) {
                            ...
                            currentBreakTotal = nextStart - currEnd; 
                            currentBreakTimePassed = currentMinutes - currEnd; 
        ...
        if (musicSchedules[currentUser] && musicSchedules[currentUser][day]) { 
            ...
            if (tMus && Object.keys(tMus).length > 0) {
                musicToday = true;
                Object.keys(tMus).map(Number).forEach(slotNum => {
        ...
        if (isMusicMode === 0 && musicToday && currentMinutes < lastMusicLessonEndMinutes) { 
            timeDiffText = `<div class="cyber-rest-box"><div class="cyber-rest-status">ЧИЛЛ!!</div><div class="music-stamp">Не забудь про музыкалку! 🎵</div></div>`;
        } else { 
            timeDiffText = `<div class="cyber-rest-box"><div class="cyber-rest-status">ЧИЛЛ!!</div></div>`;
        }
        subText = "";
    }
    ...
    document.getElementById('timer-sub').innerHTML = subText;
...
setTimeout(() => { 
    switchScreen(currentIdx); 
    if (isMusicMode === 1) { 
        const btn = document.getElementById('music-toggle-btn'); if (btn) btn.classList.add('music-active'); 
        const weekTitle = document.getElementById('week-header-title'); if (weekTitle) weekTitle.innerText = "Музыкальная неделя"; 
        buildMatrix(); updateLogic(); 
    } 
}, 120);
